...
static void update_square(struct minigb_apu_ctx *ctx, audio_sample_t *samples,
		const bool ch2, const unsigned nsamples)
{
	struct chan *c = &ctx->chans[ch2];
	const uint_fast16_t nstereo = (uint_fast16_t)(nsamples * AUDIO_CHANNELS);
	...
	for (uint_fast16_t i = 0; i < nstereo; i += 2) {
		update_len(ctx, c);
		if (!c->enabled)
			return;
...
{
	if (samples == 0 || samples > AUDIO_SAMPLES_MAX)
		return;

	memset(stream, 0, samples * AUDIO_CHANNELS * sizeof(audio_sample_t));
	update_square(ctx, stream, 0, samples);
	update_square(ctx, stream, 1, samples);
	update_wave(ctx, stream, samples);
	update_noise(ctx, stream, samples);
}
