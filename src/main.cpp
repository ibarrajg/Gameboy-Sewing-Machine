...
static constexpr uint8_t JN100_SIMULATED_RESPONSE = 0xA5;
static volatile bool jn100_simulator_active = false;
static volatile uint8_t jn100_simulator_bit_index = 0;
static uint8_t jn100_simulator_tx_original = 0;
static uint8_t jn100_simulator_tx_shift = 0;
static uint8_t jn100_simulator_rx_shift = 0;
static uint8_t jn100_simulator_response_shift = 0;
static uint8_t jn100_simulator_tx_reconstructed = 0;
static volatile uint32_t jn100_simulator_clock_pulses = 0;
static volatile uint32_t jn100_simulator_bytes_completed = 0;
static volatile uint32_t jn100_simulator_errors = 0;
static volatile uint8_t jn100_simulator_last_tx = 0;
static volatile uint8_t jn100_simulator_last_rx = 0;
...
static bool jn100_simulator_start_next_byte()
{
    if (jn100_simulator_active) {
        return true;
    }

    uint8_t tx_byte = 0;
    if (!jn100_transport_dequeue(&tx_byte)) {
        return false;
    }

    jn100_simulator_tx_original = tx_byte;
    jn100_simulator_tx_shift = tx_byte;
    jn100_simulator_rx_shift = 0;
    jn100_simulator_response_shift = JN100_SIMULATED_RESPONSE;
    jn100_simulator_tx_reconstructed = 0;
    jn100_simulator_bit_index = 7;
    jn100_simulator_active = true;
    return true;
}

static void jn100_simulator_clock_pulse()
{
    if (!jn100_simulator_active) {
        return;
    }

    const uint8_t tx_bit =
        (jn100_simulator_tx_shift & 0x80) ? 1 : 0;
    const uint8_t rx_bit =
        (jn100_simulator_response_shift & 0x80) ? 1 : 0;

    jn100_simulator_tx_reconstructed =
        static_cast<uint8_t>((jn100_simulator_tx_reconstructed << 1) |
                             tx_bit);
    jn100_simulator_rx_shift =
        static_cast<uint8_t>((jn100_simulator_rx_shift << 1) | rx_bit);
    jn100_simulator_tx_shift <<= 1;
    jn100_simulator_response_shift <<= 1;
    ++jn100_simulator_clock_pulses;

    if (jn100_simulator_bit_index == 0) {
        jn100_simulator_last_tx = jn100_simulator_tx_reconstructed;
        jn100_simulator_last_rx = jn100_simulator_rx_shift;
        ++jn100_simulator_bytes_completed;

        if (jn100_simulator_tx_reconstructed !=
                jn100_simulator_tx_original ||
            jn100_simulator_rx_shift != JN100_SIMULATED_RESPONSE) {
            ++jn100_simulator_errors;
        }

        jn100_simulator_active = false;
        return;
    }

    --jn100_simulator_bit_index;
}

static void jn100_simulator_drain_queue()
{
    while (jn100_simulator_active || jn100_transport_depth != 0) {
        if (!jn100_simulator_start_next_byte()) {
            return;
        }
        jn100_simulator_clock_pulse();
    }
}
...
static void jn100_transport_reset()
{
    ...
    jn100_simulator_active = false;
    jn100_simulator_bit_index = 0;
    jn100_simulator_tx_original = 0;
    jn100_simulator_tx_shift = 0;
    jn100_simulator_rx_shift = 0;
    jn100_simulator_response_shift = 0;
    jn100_simulator_tx_reconstructed = 0;
    jn100_simulator_clock_pulses = 0;
    jn100_simulator_bytes_completed = 0;
    jn100_simulator_errors = 0;
    jn100_simulator_last_tx = 0;
    jn100_simulator_last_rx = 0;
}
...
static void jn100_serial_tx(struct gb_s *gb_ctx, const uint8_t tx)
{
    ...
    if (replacement_payload) {
        /* Queue every meaningful ROM byte, then exercise eight simulated
         * external clock pulses per byte. No GPIO is used in this backend. */
        if (jn100_transport_enqueue(tx)) {
            jn100_simulator_drain_queue();
        }
...
static void setupWebServer() {
  ...
  server.on("/api/jn100", HTTP_GET, [](AsyncWebServerRequest *req) {
    ...
    json += "\n  },\n  \"simulator\": {\n";
    json += "    \"active\": ";
    json += jn100_simulator_active ? "true" : "false";
    json += ",\n    \"currentBit\": ";
    json += String(static_cast<unsigned>(jn100_simulator_bit_index));
    json += ",\n    \"clockPulses\": ";
    json += String(static_cast<unsigned long>(jn100_simulator_clock_pulses));
    json += ",\n    \"bytesCompleted\": ";
    json += String(static_cast<unsigned long>(jn100_simulator_bytes_completed));
    json += ",\n    \"errors\": ";
    json += String(static_cast<unsigned long>(jn100_simulator_errors));
    json += ",\n    \"simulatedResponse\": ";
    json += String(static_cast<unsigned>(JN100_SIMULATED_RESPONSE));
    json += ",\n    \"lastTx\": ";
    json += String(static_cast<unsigned>(jn100_simulator_last_tx));
    json += ",\n    \"lastRx\": ";
    json += String(static_cast<unsigned>(jn100_simulator_last_rx));
    json += "\n  }";
    json += ",\n  \"packet\": [";
