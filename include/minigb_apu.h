...
/* Truncated samples/frame (548 at 32768 Hz). Exact rate is 548.625. */
#define AUDIO_SAMPLES		((unsigned)(AUDIO_SAMPLE_RATE / VERTICAL_SYNC))
/* Room for Bresenham +1 sample frames so audio time does not drift. */
#define AUDIO_SAMPLES_MAX	(AUDIO_SAMPLES + 1)
/* Number of audio channels. The audio output is in interleaved stereo format.*/
#define AUDIO_CHANNELS		2
/* Stereo interleaved count for a max-sized callback. */
#define AUDIO_SAMPLES_TOTAL	(AUDIO_SAMPLES_MAX * AUDIO_CHANNELS)
...
/**
 * Fill allocated buffer "stream" with `samples` stereo frames of 16-bit
 * signed PCM (native endian, interleaved L,R).
 * Each call should cover one Game Boy VSYNC; pass 548 or 549 at 32768 Hz
 * (via a fractional accumulator) so average rate stays exact.
 *
 * \param ctx Library context. Must be initialised with audio_init().
 * \param stream Buffer of at least (samples * AUDIO_CHANNELS) samples.
 * \param samples Mono-frame count to generate (typically AUDIO_SAMPLES or +1).
 */
void minigb_apu_audio_callback(struct minigb_apu_ctx *ctx,
		audio_sample_t *stream, unsigned samples);
