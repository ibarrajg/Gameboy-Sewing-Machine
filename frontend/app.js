(() => {
  ...
  function playPcmU8(pcm) {
    ...
    // Overrun: drop this chunk so latency doesn't balloon
    if (nextAudioTime > now + AUDIO_MAX_AHEAD) {
      return;
    }
  ...
  function enqueueFrame(video, audio) {
    // Schedule audio on arrival so video drops / jitter do not chop the soundtrack
    if (audio && audio.length) {
      playPcmU8(audio);
    }
    // Own the bytes — WS ArrayBuffers are recycled by the browser
    frameQueue.push({
